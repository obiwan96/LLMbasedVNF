...
def make_mop(args):
    ...
    num_mop_per_vnf = int(args.num/5)
    ...
    for function in function_list:
        make_mop_count=0
        while make_mop_count<num_mop_per_vnf:
            for lang in langs:
                pattern = re.compile(
                    rf"{re.escape(function)}_setup_.*_{re.escape(lang)}_(\d+)\.docx$"
                )
                last_num=0
                ...
                for additional_command in additional_command_list[function]:
                    for prompt in prompts['mop']:
                        if make_mop_count>=num_mop_per_vnf:
                            break
                        ...
                        if args.model == 'gemini':
                            response = gemini_client.models.generate_content(
                                model="gemini-3-flash-preview", contents=formatted_prompt
                            )
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--num', type=int, required=True, help='Number of MOPs to generate')
    ...
    argparser.add_argument('--model', choices = ['gpt', 'gemini'],required=True, help='Choose the model to generate MOP files.')
