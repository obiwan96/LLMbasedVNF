# Original code that use Huffing Face TRL library.
...
def train_grpo(
    ...
    top_p: float = 0.9,
    steps: int = 80,
    batch_size: int = 2,               # PPOConfig.batch_size 대응 (per_device_train_batch_size)
    grad_accum: int = 8,               # PPO의 gradient_accumulation_steps 대응
    ...
    print(f"[INFO] Starting GRPO training with reduced memory settings.")
    gc.collect()
    torch.cuda.empty_cache()
    ...
    args = GRPOConfig(
        ...
        # Disable gradient checkpointing to avoid warnings
...
def train_grpo_trajectory(
    ...
    def trajectory_reward_func(prompts, completions, vnf, trainer_state=None, **kwargs):
        ...
        for prompt, comp, v in zip(prompts, completions, vnf):
            ...
            r1, _ = get_reward_from_llm_response(a1, form, v, model_name,{v: 1}, k8s_client, namespace, logging=False, use_whole_code=use_whole_code)
            if r1 == 1.0:
            ...
            if a2 is not None:
                ...
                r2, _ = get_reward_from_llm_response(a2, form, v, model_name,{v: 1}, k8s_client, namespace, logging=False, use_whole_code=use_whole_code)
    ...
    args = GRPOConfig(
        ...
        max_prompt_length=max_prompt_length,#+max_new_tokens*2, # to make a2, we should put a1 and rag result in prompt
        generation_batch_size=num_generations * num_prompts_per_step,
        num_generations=num_generations,
        ...
        def _generate(self, prompts: list[str], images: Optional[list] = None):
            ...
            for i in range(len(prompts)):
                ...
                r1, test_result = get_reward_from_llm_response(
                    a1_text, form, vnf, model_name, {vnf:1}, k8s_client, namespace, logging=True, use_whole_code=use_whole_code
                )
                ...
                if r1 == 1.0:
                    ...
                    generation_config.max_prompt_length = max_prompt_length+max_new_tokens*2
                    generation_config.max_completion_length = max_new_tokens*2
                    generation_config.max_new_tokens = max_new_tokens * 2
...
def train_dpo(
    ...
    # 1) Dataset 구성
    max_prompt_length = max_prompt_length
    max_length=max_prompt_length+max_context_length
    if using_cot:
...
def _load_hf_trajectory_model(model_path_or_id: str, hf_cache_path: str, lora_config: LoraConfig):
    adapter_config_path = os.path.join(model_path_or_id, "adapter_config.json")
    tokenizer_source = model_path_or_id
    ...
    if os.path.isdir(model_path_or_id) and os.path.exists(adapter_config_path):
        peft_config = PeftConfig.from_pretrained(model_path_or_id)
        base_model_path = peft_config.base_model_name_or_path
        print(f"[INFO] Loading HF trajectory model from PEFT adapter: {model_path_or_id}")
        print(f"[INFO] Adapter base model: {base_model_path}")
        base_model = AutoModelForCausalLM.from_pretrained(
            base_model_path,
            cache_dir=hf_cache_path,
        ...
        model = PeftModel.from_pretrained(base_model, model_path_or_id, is_trainable=True)
        if not os.path.exists(os.path.join(model_path_or_id, "tokenizer_config.json")):
            tokenizer_source = base_model_path
    else:
        print(f"[INFO] Loading HF trajectory model directly from: {model_path_or_id}")
        model = AutoModelForCausalLM.from_pretrained(
            model_path_or_id,
            cache_dir=hf_cache_path,
            quantization_config=bnb_config,
    ...
    def _hf_for_training(self, use_gradient_checkpointing=True):
        self.train()
        if hasattr(self, "gradient_checkpointing_enable") and use_gradient_checkpointing:
            self.gradient_checkpointing_enable()
        if hasattr(self, "gradient_checkpointing_disable") and not use_gradient_checkpointing:
            self.gradient_checkpointing_disable()
        if hasattr(self, "config"):
            self.config.use_cache = False
        return self

    def _hf_for_inference(self):
        self.eval()
        if hasattr(self, "config"):
            self.config.use_cache = True
        return self

    model.for_training = _hf_for_training.__get__(model, type(model))
    model.for_inference = _hf_for_inference.__get__(model, type(model))
    if not hasattr(model, "warnings_issued") or getattr(model, "warnings_issued") is None:
        model.warnings_issued = {}
    if hasattr(model, "base_model"):
        model.base_model.for_training = _hf_for_training.__get__(model.base_model, type(model.base_model))
        model.base_model.for_inference = _hf_for_inference.__get__(model.base_model, type(model.base_model))
        if not hasattr(model.base_model, "warnings_issued") or getattr(model.base_model, "warnings_issued") is None:
            model.base_model.warnings_issued = {}
        if hasattr(model.base_model, "model") and (
            not hasattr(model.base_model.model, "warnings_issued") or getattr(model.base_model.model, "warnings_issued") is None
        ):
            model.base_model.model.warnings_issued = {}
    elif hasattr(model, "model") and (
        not hasattr(model.model, "warnings_issued") or getattr(model.model, "warnings_issued") is None
    ):
...
def train_grpo_trajectory_based_unsloth(
    model_path_or_id: str,
    mop_data: List[Dict[str, str]],
    k8s_client,
    form: str = "Python",
    ...
    model, tokenizer = _load_hf_trajectory_model(model_path_or_id, hf_cache_path, lora_config)
    ...
    train_dataset = Dataset.from_list(train_rows)
    model_name = model_path_or_id.split("/")[-1] + "_GRPO_traj_HF"
    writer = SummaryWriter(output_dir_logs)
    _cache = {"step": None, "raw": [], "shaped": [], "a2_used_usage": []}
    logging_file = 'tmp/rl_llm_response_trajectory_a2_log.txt'
    ...
    def trajectory_reward_func(prompts, completions, vnf, trainer_state=None, **kwargs):
        ...
        for prompt, comp, v in zip(prompts, completions, vnf):
            ...
            if using_cot:
                a1 = parse_reasoning_and_final(a1)[1]
            r1, _ = get_reward_from_llm_response(
                a1, form, v, model_name, {v: 1}, k8s_client, namespace, logging=False, use_whole_code=use_whole_code
            )
            if r1 == 1.0:
                r1 += bonus_reward
            ...
            if a2 is not None:
                if using_cot:
                    a2 = parse_reasoning_and_final(a2)[1]
                r2, _ = get_reward_from_llm_response(
                    a2, form, v, model_name, {v: 1}, k8s_client, namespace, logging=False, use_whole_code=use_whole_code
                )
                r2 -= cost_reward
    ...
    class TrajectoryGRPOTrainerHF(GRPOTrainer):
        def _generate(self, prompts: list[str], images: Optional[list] = None):
            device = self.accelerator.device
            mode = "train" if self.model.training else "eval"
            ...
            for i in range(len(prompts)):
                ...
                if r1 == 1.0:
                    ...
                    status_code, server_or_message = test_result
                    inform_msg, req_msg, err_msg = select_request_message(
                        status_code, form, vnf, server_or_message, collection, embed_model
                    )
                    rag_prompt = clean_prompt + a1_text + "\n" + inform_msg + err_msg + req_msg
                    ...
                    if rag_token_count > self.args.max_prompt_length:
                        print(
                            f"     WARNING: Prompt length ({rag_token_count}) exceeds max_prompt_length ({self.args.max_prompt_length})!"
                        )
    ...
    trainer = TrajectoryGRPOTrainerHF(
        model=model,
        args=args,
        reward_funcs=trajectory_reward_func,
        train_dataset=train_dataset,
    ...
    print(f"✅ HF Trajectory GRPO training finished: {output_dir}")
...
if __name__ == '__main__':
    ...
    max_new_tokens = 1024 + 500 if argparser.cot else 1024
